defmodule MehungryServerWeb.MeasurementUnitController do
  ...
  def create(conn, %{"measurement_unit" => measurement_unit_params}) do
    with {:ok, %MeasurementUnit{} = measurement_unit} <- Food.create_measurement_unit(measurement_unit_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "measurement_unit" => measurement_unit_params}) do
    ...
    with {:ok, %MeasurementUnit{} = measurement_unit} <- Food.update_measurement_unit(measurement_unit, measurement_unit_params) do
      render(conn, "show.json", measurement_unit: measurement_unit)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
   ...
   def swagger_definitions do
    %{
    MeasurementUnit: swagger_schema do
      title "Measurement Unit"
      description "Structs representing measurement units used to associate ingredients with recipes" 
      properties do
        name :string, "The name of the measurement unit e.g grammar", required: true 
        translation Schema.ref(:MeasurementUnitTranslation)
      end
    end, 
    MeasurementUnitTranslation: swagger_schema do
      title "Measurement Unit Transalation"
      description "Structs representing measurement units used to associate ingredients with recipes" 
      properties do
        name :string, "The name of the measurement unit translated in the language infered e.g grammar", required: true 
        language Schema.ref(:Language)
      end
    end,
    Language: swagger_schema do
      title "Language"
