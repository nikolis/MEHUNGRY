defmodule MehungryServerWeb.LoginController do
  ...
  alias MehubgryServer.Credentials

  action_fallback MehungryServerWeb.FallbackController

  def login_with_credential(conn, %{"credential" => %{"email" => email, "password" => pass, "captcha_token" => google_token}}) do
 ...
 def login_by_facebook_token(conn, %{"user_id"=> user_id, "token"=> token}) do
  ...
  def fb_token_validation(token) do
    uri = URI.to_string(%URI{
      ...
      path: "debug_token?input_token=#{token}&access_token=902631176539278|CBzcAzROucHU1sX8tWBlObKvHc4"
    })
  end
      ...
      {:ok, %HTTPoison.Response{status_code: 200, body: body}} ->
        ...
        case data_map do
          %{"app_id" => "902631176539278", "is_valid" => true} ->
            user_details = fb_user_details(token)
              |> HTTPoison.get
