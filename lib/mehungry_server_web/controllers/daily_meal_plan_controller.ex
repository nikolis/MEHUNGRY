defmodule MehungryServerWeb.DailyMealPlanController do
  ...
  def create(conn, %{"daily_meal_plan" => daily_meal_plan_params}) do
    with {:ok, %DailyMealPlan{} = daily_meal_plan} <- Plans.create_daily_meal_plan(daily_meal_plan_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "daily_meal_plan" => daily_meal_plan_params}) do
    ...
    with {:ok, %DailyMealPlan{} = daily_meal_plan} <- Plans.update_daily_meal_plan(daily_meal_plan, daily_meal_plan_params) do
      render(conn, "show.json", daily_meal_plan: daily_meal_plan)
    end
  end
